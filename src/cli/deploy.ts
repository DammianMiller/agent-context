...
async function queueDeploy(batcher: DeployBatcher, options: DeployOptions): Promise<void> {
  ...
  if (!agentId || !actionType || !target) {
    ...
    console.log(chalk.dim('Example: uam deploy queue --agent-id <id> --action-type commit --target main'));
    process.exit(1);
  }
...
async function createBatch(batcher: DeployBatcher, options: DeployOptions): Promise<void> {
  ...
  try {
    ...
    console.log(chalk.cyan(`\n  Execute with: uam deploy execute --batch-id ${batch.id}`));
  } catch (error) {
...
async function showDeployStatus(options: DeployOptions): Promise<void> {
  ...
  try {
    ...
    if (totalPending > 0) {
      console.log(chalk.dim('Run "uam deploy batch" to create a batch from pending actions'));
    }
    if (pendingBatches.length > 0) {
      console.log(chalk.dim(`Run "uam deploy execute --batch-id <id>" to execute a batch`));
    }
    if (totalPending > 0 || totalBatched > 0) {
      console.log(chalk.dim('Run "uam deploy flush" to execute all pending deploys'));
    }
  } catch (error) {
